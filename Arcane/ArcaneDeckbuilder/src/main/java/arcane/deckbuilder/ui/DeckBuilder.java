...
public class DeckBuilder extends DeckBuilderUI {
    ...
    /**
     * <p>Constructor for DeckBuilder.</p>
     */
    public DeckBuilder() {
    ...
    /**
     * <p>Getter for the field <code>currentDecklist</code>.</p>
     *
     * @return a {@link arcane.Decklist} object.
     */
     ...
     * @param sideboard a boolean.
     */
    public void addDecklistCard(Card card, boolean sideboard) {
        CardTable table = (sideboard ? getSideTable() : getDeckTable());
     ...
     * <p>saveDecklist</p>
     *
     * @param decklist a {@link arcane.Decklist} object.
     * @param updateMRU a boolean.
     */
    public void saveDecklist(Decklist decklist, boolean updateMRU) {
        ...
        try {
            decklist.save(getDeckTable().model.viewCards, getDeckTable().model.cardToQty, getSideTable().model.viewCards, getSideTable().model.cardToQty);
    ...
    public void loadDecklist(Decklist decklist) {
        getDeckTable().model.unsortedCards = new ArrayList<Card>();
        getDeckTable().model.viewCards = new ArrayList<Card>();
        getSideTable().model.unsortedCards = new ArrayList<Card>();
        getSideTable().model.viewCards = new ArrayList<Card>();
        try {
            ...
            history.clear();
            historyIndex = -1;
            getDeckTable().model.fireTableDataChanged();
            getSideTable().model.fireTableDataChanged();
            getDeckTable().sort();
            getSideTable().sort();
            updateDeckCount();
            if (decklist != null && decklist.isOpenable()) {
    ...
    private void loadPreferences() {
        ...
        getDeckTable().setPreferences(deckTableValues);
    ...
    protected void savePreferences() {
        ...
        Map<String, String> deckTableValues = getDeckTable().getPreferences();
    ...
    public void updateCardTable() {
        ...
        Util.threadPool.submit(new Runnable() {
            public void run() {
                ...
                if (artifactButton.isSelected())
                    where.append(" OR (color = 'artifact' OR englishType LIKE '%artifact%')");
                ...
                if (enchantButton.isSelected())
                    where.append(" OR (englishType LIKE '%enchantment%' AND type NOT LIKE '%aura%')");
    ...
    public List<Card> getDeckCards() {
        return getDeckTable().model.viewCards;
    }

    ;

    /**
     * <p>getDeckCardToQty</p>
     *
     * @return a {@link java.util.Map} object.
     */
    public Map<Card, Integer> getDeckCardToQty() {
        return getDeckTable().model.cardToQty;
    }
    ...
    public List<Card> getSideCards() {
        return getSideTable().model.viewCards;
    }

    ;

    /**
     * <p>getSideCardToQty</p>
     *
     * @return a {@link java.util.Map} object.
     */
    public Map<Card, Integer> getSideCardToQty() {
        return getSideTable().model.cardToQty;
    }
     ...
     * <p>addPluginMenu</p>
     *
     * @param pluginMenu a {@link javax.swing.JMenu} object.
     */
    ...
    private boolean addCardToTable(Card card, CardTable table) {
        ...
        int existingQty = getDeckTable().getQty(card) + getSideTable().getQty(card);
    ...
    private void updateDeckCount() {
        ...
        deckStats.deckCount = getDeckTable().model.unsortedCards.size();
        deckStats.sideCount += getSideTable().model.unsortedCards.size();
        UI.setTitle(deckGroup, "Deck (" + deckStats.deckCount + "/" + deckStats.sideCount + ")");

        int totalSpells = 0;
        int totalConvertedCost = 0;
        for (Card card : getDeckTable().model.unsortedCards) {
            ...
            outerLoop:
            for (int ii = 0, n = card.castingCost.length(); ii < n; ii++) {
        ...
        if (totalSpells > 0)
            deckStats.avg = String.valueOf(Math.round((totalConvertedCost / (float) totalSpells) * 100) / 100F);

        Set<String> sets;
        sets = arcane.getFormatSets(Format.vintage);
        deckStats.illegalT1main = isIllegal(Format.vintage, sets, getDeckTable(), false);
        deckStats.illegalT1side = isIllegal(Format.vintage, sets, getSideTable(), false);
        sets = arcane.getFormatSets(Format.legacy);
        deckStats.illegalT15main = isIllegal(Format.legacy, sets, getDeckTable(), false);
        deckStats.illegalT15side = isIllegal(Format.legacy, sets, getSideTable(), false);
        sets = arcane.getFormatSets(Format.extended);
        deckStats.illegalT1Xmain = isIllegal(Format.extended, sets, getDeckTable(), false);
        deckStats.illegalT1Xside = isIllegal(Format.extended, sets, getSideTable(), false);
        sets = arcane.getFormatSets(Format.standard);
        deckStats.illegalT2main = isIllegal(Format.standard, sets, getDeckTable(), false);
        deckStats.illegalT2side = isIllegal(Format.standard, sets, getSideTable(), false);
        ...
        getSideTable().illegalCards = getDeckTable().illegalCards;
        getDeckTable().illegalCards.clear();
        sets = new HashSet();
        for (Object entry : setsList.getSelectedValues())
            sets.add(((SetEntry) entry).set);
        isIllegal(currentFormat, sets, getDeckTable(), true);
        isIllegal(currentFormat, sets, getSideTable(), true);
        if (!getDeckTable().illegalCards.isEmpty()) {
            StringBuffer buffer = new StringBuffer(256);
            buffer.append("Illegal cards: ");
            int i = 0, lastIndex = getDeckTable().illegalCards.size() - 1;
            for (Iterator iter = getDeckTable().illegalCards.iterator(); iter.hasNext();) {
    ...
    private boolean isIllegal(Format format, Set<String> sets, CardTable table, boolean setIllegalCards) {
        ...
        if (setIllegalCards)
            return !getDeckTable().illegalCards.isEmpty();
        else
            return false;
    }
    ...
    private void updateButtons() {
        ...
        for (Card card : cardsTable.getSelectedCards()) {
            if (!isTooMany(card, getDeckTable().getQty(card) + getSideTable().getQty(card))) {
                canAddToDeck = true;
                break;
            }
        }
        ...
        canAddToDeck = false;
        for (Card card : getDeckTable().getSelectedCards()) {
            if (!isTooMany(card, getDeckTable().getQty(card) + getSideTable().getQty(card))) {
                canAddToDeck = true;
                break;
            }
        }
        for (Card card : getSideTable().getSelectedCards()) {
            if (!isTooMany(card, getDeckTable().getQty(card) + getSideTable().getQty(card))) {
                canAddToDeck = true;
                break;
            }
        ...
        boolean isCardSelected = getDeckTable().getSelectedRow() != -1 || getSideTable().getSelectedRow() != -1;
        removeDeckButton.setEnabled(isCardSelected);
        swapDeckButton.setEnabled(isCardSelected);

        CardTable table = getDeckTable().getSelectedRow() != -1 ? getDeckTable() : null;
        if (table == null) table = getSideTable().getSelectedRow() != -1 ? getSideTable() : null;
        ...
        reorderDeckButton.setEnabled((getDeckTable().model.viewCards.size() > 1 || getSideTable().model.viewCards.size() > 1)
                && getDeckTable().sortedColumns.size() == 0 && getSideTable().sortedColumns.size() == 0);
        unsortDeckButton.setEnabled(getDeckTable().sortedColumns.size() > 0 || getSideTable().sortedColumns.size() > 0);
    }
    ...
    private void initializePopupMenus() {
        ...
        addCommonMenus(deckTableCardMenu, getDeckTable());
        ...
        addCommonMenus(sideTableCardMenu, getSideTable());
    }
    ...
    private void initializeEvents() {
        ...
        actionMap.put("focusDeckTable", new AbstractAction() {
            public void actionPerformed(ActionEvent evt) {
                if (getDeckTable().isFocusOwner()) {
                    getSideTable().requestFocus();
                    if (getSideTable().getSelectedRow() == -1) selectFirstVisibleRow(getSideTable());
                } else {
                    getDeckTable().requestFocus();
                    if (getDeckTable().getSelectedRow() == -1) selectFirstVisibleRow(getDeckTable());
                }
            }
        });
            ...
            public void keyPressed(KeyEvent evt) {
                switch (evt.getKeyCode()) {
                    ...
                    case KeyEvent.VK_PAGE_DOWN:
                        ...
                        break;
                    default:
                        if (lastQuickSearch == null || !lastQuickSearch.equals(quickSearchText.getText()))
                            quickSearch(false);
                }
            }
        });
        ...
        addCardButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                DeckAction action = new DeckAction(getDeckTable(), Action.add);
                for (int i = 0, n = addCardButton.isAlternate(evt) ? 4 : 1; i < n; i++)
                    for (Card card : cards)
                        if (addCardToTable(card, getDeckTable())) action.addCard(card);
                setCurrentDecklist(getCurrentDecklist());
                addHistory(action);
                updateDeckCount();
                getDeckTable().sort();
                getDeckTable().setSelectedCards(cards);
                getDeckTable().requestFocus();
                updateButtons();
            }
        });
        addSideCardButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                List<Card> cards = cardsTable.getSelectedCards();
                DeckAction action = new DeckAction(getSideTable(), Action.add);
                for (int i = 0, n = addSideCardButton.isAlternate(evt) ? 4 : 1; i < n; i++)
                    for (Card card : cards)
                        if (addCardToTable(card, getSideTable())) action.addCard(card);
                setCurrentDecklist(getCurrentDecklist());
                addHistory(action);
                updateDeckCount();
                getSideTable().sort();
                getSideTable().setSelectedCards(cards);
                getSideTable().requestFocus();
                updateButtons();
            }
        });
        ...
        cardsTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent evt) {
                ...
                getDeckTable().clearSelection();
                if (card != null) {
                    getDeckTable().setSelectedCards(card.name);
                    if (getDeckTable().getSelectedRow() == -1) {
                        getSideTable().clearSelection();
                        getSideTable().setSelectedCards(card.name);
                    }
                }
        ...
        cardsTable.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent evt) {
                ...
                CardTable toTable = evt.getButton() == 1 ? getDeckTable() : getSideTable();
            ...
            public void keyPressed(KeyEvent evt) {
                ...
                switch (evt.getKeyCode()) {
                    ...
                    case KeyEvent.VK_HOME:
                        ...
                        if (cardsTable.model.viewCards.size() > 0)
                            cardsTable.setSelectedCard(cardsTable.model.viewCards.get(0));
                        break;
                    ...
                    case KeyEvent.VK_CONTROL:
                        break;
                    case KeyEvent.VK_TAB:
                        if (evt.isShiftDown() && !getSideTable().model.viewCards.isEmpty()) {
                            getSideTable().requestFocus();
                            if (getSideTable().getSelectedRow() == -1) selectFirstVisibleRow(getSideTable());
                        } else {
                            getDeckTable().requestFocus();
        ...
        addDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                CardTable table = getDeckTable().getSelectedCard() != null ? getDeckTable() : getSideTable();
        ...
        swapDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                CardTable fromTable = getDeckTable().getSelectedCard() != null ? getDeckTable() : getSideTable();
                CardTable toTable = fromTable == getSideTable() ? getDeckTable() : getSideTable();
        ...
        upDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                List<Card> cards = table.getSelectedCards();
                outerLoop:
                for (int i = 0, n = upDeckButton.isAlternate(evt) ? 4 : 1; i < n; i++) {
        ...
        downDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                List<Card> cards = table.getSelectedCards();
                outerLoop:
                for (int i = 0, n = downDeckButton.isAlternate(evt) ? 4 : 1; i < n; i++) {
                    List<Integer> indices = new ArrayList();
        ...
        reorderDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                Collections.sort(getDeckTable().model.unsortedCards, comparator);
                getDeckTable().sort();
                Collections.sort(getSideTable().model.unsortedCards, comparator);
                getSideTable().sort();
                updateButtons();
            }
        });
        unsortDeckButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                getDeckTable().sortedColumns.clear();
                getDeckTable().sort();
                getSideTable().sortedColumns.clear();
                getSideTable().sort();
                updateButtons();
            }
        });
            ...
            public void keyPressed(KeyEvent evt) {
                ...
                switch (keyCode) {
                    ...
                    case KeyEvent.VK_END:
                        ...
                        break;
                    case KeyEvent.VK_TAB:
                        if (evt.isShiftDown() && table == getSideTable()) {
                            getDeckTable().requestFocus();
                            if (getDeckTable().getSelectedRow() == -1) selectFirstVisibleRow(getDeckTable());
                        } else if (!evt.isShiftDown() && table == getDeckTable() && !getSideTable().model.viewCards.isEmpty()) {
                            getSideTable().requestFocus();
                            if (getSideTable().getSelectedRow() == -1) selectFirstVisibleRow(getSideTable());
                        } else {
                            cardsTable.requestFocus();
        ...
        getDeckTable().getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent evt) {
                if (evt.getValueIsAdjusting()) return;
                if (ignoreSelections) return;
                Card card = getDeckTable().getSelectedCard();
                cardInfoPane.setCard(card);
                ignoreSelections = true;
        ...
        getDeckTable().addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent evt) {
                List<Card> cards = getDeckTable().getSelectedCards();

                if (evt.getClickCount() == 1 && evt.getButton() == 3) {
                    setCardCountMenuItem(deckTableCardCountItem, cards);
                    deckTableCardMenu.show(getDeckTable(), evt.getX() + 1, evt.getY() + 1);
                    return;
                }

                if (evt.getClickCount() < 2 || evt.getClickCount() % 2 != 0) return;
                DeckAction addAction = new DeckAction(evt.getButton() == 1 ? getDeckTable() : getSideTable(), Action.add);
                DeckAction removeAction = new DeckAction(getDeckTable(), Action.remove);
                for (int i = 0, n = evt.isAltDown() ? 4 : 1; i < n; i++) {
                    for (Card card : cards) {
                        if (evt.getButton() == 1) {
                            if (addCardToTable(card, getDeckTable())) addAction.addCard(card);
                        } else if (evt.getButton() == 3) {
                            boolean removed = removeCardFromTable(card, getDeckTable());
                            if (removed) {
                                removeAction.addCard(card);
                                if (evt.isControlDown()) {
                                    if (addCardToTable(card, getSideTable())) addAction.addCard(card);
                                }
                            }
                        }
                    }
                ...
                addHistory(removeAction);
                updateDeckCount();
                if (evt.getButton() == 3 && evt.isControlDown()) getSideTable().sort();
                getDeckTable().sort();
                updateButtons();
            }
        });
        getDeckTable().getModel().addTableModelListener(new TableModelListener() {
            public void tableChanged(TableModelEvent evt) {
                updateButtons();
            }
        });
        getDeckTable().addFocusListener(new FocusAdapter() {
            public void focusGained(FocusEvent evt) {
                cardInfoPane.setCard(getDeckTable().getSelectedCard());
            }
        });
        getDeckTable().addKeyListener(tableKeyListener);

        getSideTable().getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent evt) {
                if (evt.getValueIsAdjusting()) return;
                if (ignoreSelections) return;
                Card card = getSideTable().getSelectedCard();
                cardInfoPane.setCard(card);
                ignoreSelections = true;
                cardsTable.clearSelection();
                if (linkTablesButton.isSelected() && card != null) {
        ...
        getSideTable().addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent evt) {
                List<Card> cards = getSideTable().getSelectedCards();

                if (evt.getClickCount() == 1 && evt.getButton() == 3) {
                    setCardCountMenuItem(sideTableCardCountItem, cards);
                    sideTableCardMenu.show(getSideTable(), evt.getX() + 1, evt.getY() + 1);
                    return;
                }

                if (evt.getClickCount() < 2 || evt.getClickCount() % 2 != 0) return;
                DeckAction addAction = new DeckAction(evt.getButton() == 1 ? getSideTable() : getDeckTable(), Action.add);
                DeckAction removeAction = new DeckAction(getSideTable(), Action.remove);
                for (int i = 0, n = evt.isAltDown() ? 4 : 1; i < n; i++) {
                    for (Card card : cards) {
                        if (evt.getButton() == 1) {
                            if (addCardToTable(card, getSideTable())) addAction.addCard(card);
                        } else if (evt.getButton() == 3) {
                            boolean removed = removeCardFromTable(card, getSideTable());
                            if (removed) {
                                removeAction.addCard(card);
                                if (evt.isControlDown()) {
                                    if (addCardToTable(card, getDeckTable())) addAction.addCard(card);
                ...
                addHistory(removeAction);
                updateDeckCount();
                if (evt.getButton() == 3 && evt.isControlDown()) getDeckTable().sort();
                getSideTable().sort();
                updateButtons();
            }
        });
        getSideTable().getModel().addTableModelListener(new TableModelListener() {
            public void tableChanged(TableModelEvent evt) {
                updateButtons();
            }
        });
        getSideTable().addFocusListener(new FocusAdapter() {
            public void focusGained(FocusEvent evt) {
                cardInfoPane.setCard(getSideTable().getSelectedCard());
            }
        });
        getSideTable().addKeyListener(tableKeyListener);
        ...
        saveMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                if (getCurrentDecklist() != null) {
                    saveDecklist(getCurrentDecklist(), true);
                    return;
                }
        ...
        presetCombo.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                if (sets == null) {
                    if (presetCombo.getSelectedIndex() == 0)
        ...
        linkTablesButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                List<Card> cards = getDeckTable().getSelectedCards();
                if (cards.size() == 0) cards = getSideTable().getSelectedCards();
    ...
    static public void updateRows(String cardName) {
        for (DeckBuilder deckBuilder : instances) {
            ...
            for (int rowIndex : deckBuilder.getDeckTable().model.getRowIndices(cardName))
                deckBuilder.getDeckTable().model.fireTableRowsUpdated(rowIndex, rowIndex);
            for (int rowIndex : deckBuilder.getSideTable().model.getRowIndices(cardName))
                deckBuilder.getSideTable().model.fireTableRowsUpdated(rowIndex, rowIndex);
        }
    }
    ...
    private class DeckStats {
        ...
        public int getCount(CardTable table) {
            if (table == getDeckTable()) return deckCount;
            return sideCount;
        }
    }
